...
- **Inbox encryption keys:** X25519 inbox secrets are derived client-side from a Ready `signMessage` over Wotta TypedData (`inbox-key-v1`), domain-separated by chain via HKDF. Recovery assumes Ready returns a stable signature for the same TypedData. Wotta servers store only the published public key and its derivation scheme, never the secret. Reconnecting with a matching legacy browser secret auto-upgrades to Ready-derived keys; Account can also upgrade explicitly. Both are allowed while older notes are still claimable; the upgrading browser retains its previous local secret as a keyring so recoverable older notes can still be claimed. Notes whose old secret was already lost remain sealed until expiry/refund. Unlinking or reclaiming a Ready address from another profile remains blocked while live notes exist.
