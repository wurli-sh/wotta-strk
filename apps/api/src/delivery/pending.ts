...
export function assertRecipientInboxKey(expected: string, supplied: string): void {
  if (expected !== supplied) throw new Error("recipient_inbox_key_changed");
}
export async function storeDelivery(db: Db, config: Config, params: { senderId: string; intentId: string; recipient: { provider: RecipientProvider; identifier: string }; recipientInboxPublicKey: string; ciphertext: string; nonce: string; ephemeralPublicKey: string; algorithm: "x25519-xsalsa20-poly1305"; expiresAt: string }) {
  ...
  if (identity) {
    ...
    assertRecipientInboxKey(wallet.inbox_pubkey, params.recipientInboxPublicKey);
    const { error } = await db.from("encrypted_notes").upsert({ recipient_profile_id: identity.profile_id, sender_profile_id: params.senderId, intent_id: params.intentId, chain_id: config.manifest.chainId, recipient_key_version: wallet.key_version ?? 1, ciphertext: params.ciphertext, nonce: params.nonce, sender_public_key: params.ephemeralPublicKey, algorithm: params.algorithm }, { onConflict: "recipient_profile_id,intent_id" }); if (error) throw error;
    return "registered" as const;
  }
  assertRecipientInboxKey(pendingDeliveryPublicKey(config), params.recipientInboxPublicKey);
