...
export async function buildServer() {
  ...
  app.addHook("onRequest", async (request, reply) => {
    if (d.config.manifest.chainId === "SN_MAIN" && /^\/v1\/(quotes|intents|notes)(?:\/|\?|$)/.test(request.url)) {
      return reply.code(400).send({ error: { code: "route_disabled", message: "route_disabled" } });
    }
  });
  ...
  app.post("/v1/quotes", async (request, reply) => { const auth = await requireAuth(d.db, request); if (!auth) return errorReply(reply, new Error("unauthorized")); if (d.config.manifest.chainId === "SN_MAIN") return errorReply(reply, new Error("route_disabled")); try { return await signQuote(d.db, d.config, auth.userId, parse(quoteSchema, request.body)); } catch (error) { return errorReply(reply, error); } });
  app.post("/v1/intents", async (request, reply) => { const auth = await requireAuth(d.db, request), key = request.headers["idempotency-key"]; if (!auth) return errorReply(reply, new Error("unauthorized")); if (d.config.manifest.chainId === "SN_MAIN") return errorReply(reply, new Error("route_disabled")); try { const body = parse(intentSchema, request.body), idempotencyKey = idempotencySchema.parse(key); const output = await idempotent(d.db, auth.userId, idempotencyKey, body, () => createIntent(d.db, auth.userId, body)); return reply.code(201).send(output); } catch (error) { return errorReply(reply, error); } });
  app.get("/v1/intents", async (request, reply) => { const auth = await requireAuth(d.db, request); if (!auth) return errorReply(reply, new Error("unauthorized")); if (d.config.manifest.chainId === "SN_MAIN") return errorReply(reply, new Error("route_disabled")); const rawLimit = Number((request.query as { limit?: string }).limit ?? 25); const limit = Number.isInteger(rawLimit) ? Math.min(Math.max(rawLimit, 1), 100) : 25; const { data, error } = await d.db.from("intents").select("id,mode,delivery_kind,denomination,route_id,state,version,source_tx_hash,onchain_state,onchain_tx_hash,onchain_block_number,expires_at,created_at,updated_at").eq("owner_id", auth.userId).order("created_at", { ascending: false }).limit(limit); return error ? errorReply(reply, error) : { intents: data ?? [] }; });
  ...
  app.get("/v1/notes", async (request, reply) => {
    const auth = await requireAuth(d.db, request);
    if (!auth) return errorReply(reply, new Error("unauthorized"));
    if (d.config.manifest.chainId === "SN_MAIN") return errorReply(reply, new Error("route_disabled"));
