...
import {
  assertCctpPilotControls,
  assertStarknetPrivateEscrowIntent,
  buildStarknetEscrowDepositPlan,
  cctpFinalityThreshold,
  quoteMatchesStoredIntent,
  sameFelt,
  sameInstant,
  transitionAllowed,
} from "../src/intents/service.ts";
import {
  ROUTES,
  routesForConfig,
  starknetEscrowInboxReady,
  verifiedEscrowPoolsForConfig,
} from "../src/routes.ts";
import {
...
import {
  decodeEscrowProjection,
  indexedDeploymentStart,
  indexedEscrowAddresses,
} from "../src/indexer/run.ts";
...
test("identity normalization rejects unverified-shaped inputs", () => {
  ...
  assert.equal(
    normalizeIdentifier("google", "User@Example.COM"),
    "user@example.com",
  );
  assert.equal(
    normalizeIdentifier("email", "User@Example.COM"),
    "user@example.com",
  );
  assert.throws(() => normalizeIdentifier("x", "not valid"));
});
test("X-only OAuth registers both its verified handle and email", () => {
  assert.deepEqual(
    collectVerifiedIdentities({
      id: "profile-x",
    ...
    [
      {
        provider: "x",
        providerSubject: "x-subject",
        normalizedIdentifier: "alice_x",
      },
      {
        provider: "email",
        providerSubject: "x-email:x-subject",
    ...
    collectVerifiedIdentities({
      ...
      identities: [
        ...
        },
        {
          id: "x-subject",
          provider: "x",
          identity_data: { user_name: "linked_x" },
        },
      ],
    }),
    [
      {
        ...
        normalizedIdentifier: "google@example.com",
      },
      {
        provider: "x",
        providerSubject: "x-subject",
        normalizedIdentifier: "linked_x",
      },
    ],
  );
});
test("intent state machine permits no terminal escape", () => {
  assert.equal(transitionAllowed("quoted", "source_submitted"), true);
  assert.equal(transitionAllowed("claimed", "refunded"), false);
  assert.equal(transitionAllowed("expired", "claimed"), false);
});
test("quote matching tolerates timestamptz and felt padding", () => {
  assert.equal(
    sameInstant("2026-08-28T18:18:22.821Z", "2026-08-28T18:18:22.821+00:00"),
    true,
  );
    ...
    quoteMatchesStoredIntent(
      {
        ...
        public_refund_recipient:
          "0x07b54db60f62d919197e17347f0e05e0dfc2ba1ce6cd5e5f992b4d95690c90d0",
      ...
      },
      {
        ...
        publicRefundRecipient:
          "0x7b54db60f62d919197e17347f0e05e0dfc2ba1ce6cd5e5f992b4d95690c90d0",
...
test("unverified route manifest is fail closed", () => {
  assert.equal(
...
test("verified Sepolia destination admits Starknet publicly but CCTP per route", () => {
  ...
  assert.equal(
    closed.find((route) => route.id === "starknet-public")?.enabled,
    true,
  );
  assert.equal(
    closed.find((route) => route.id === "starknet-private")?.enabled,
    true,
  );
  ...
  const baseOnly = routesForConfig(
    loadConfig({ ...base, CCTP_ADMITTED_ROUTES: "base" }),
  );
  assert.equal(baseOnly.find((route) => route.id === "base")?.enabled, true);
  assert.equal(
    baseOnly.find((route) => route.id === "ethereum")?.enabled,
    false,
  );
  const allCctp = routesForConfig(
    loadConfig({
      ...base,
  ...
  assert.throws(
    () => loadConfig({ ...base, CCTP_ADMITTED_ROUTES: "unknown" }),
    /unsupported_admitted_route/,
  );
});
test("mainnet private send fails closed until escrow and privacy manifests are verified", () => {
  ...
  assert.equal(
    routes.find((route) => route.id === "starknet-private")?.enabled,
    false,
  );
  for (const id of [
    "starknet-public",
...
test("mainnet verifiedEscrowPools exclude UNDEPLOYED and unapproved dens", () => {
  ...
  assert.deepEqual(pools.map((pool) => pool.denomination).sort(), [
    "100000",
    "1000000",
  ]);
  ...
  assert.equal(
    config.manifest.pools.filter((pool) => pool.address === "UNDEPLOYED")
      .length,
    3,
  );
});
...
test("indexer fails closed when no verified escrow pools remain", () => {
  ...
  assert.throws(
    () => indexedEscrowAddresses(config),
    /indexer_verified_escrow_pools_empty/,
  );
  assert.throws(
    () => indexedDeploymentStart(config),
    /indexer_deployment_block_missing/,
  );
});
...
test("mainnet private send is admitted only as registered escrow-inbox delivery", () => {
  ...
  const pool = config.manifest.pools.find(
    (candidate) => candidate.denomination === "1000000",
  )!;
  pool.address = "0x789";
  ...
  assert.equal(
    routesForConfig(config).find((route) => route.id === "starknet-private")
      ?.enabled,
    true,
  );
  assert.deepEqual(
    buildStarknetEscrowDepositPlan(config, {
      denomination: "1000000",
      claimHash: "0x111",
  ...
  assert.doesNotThrow(() =>
    assertStarknetPrivateEscrowIntent(config, {
      routeId: "starknet-private",
      mode: "private",
      deliveryKind: "registered",
      publicRefundRecipient: "0x123",
    }),
  );
  assert.throws(
...
test("API rejects a network-mode header that does not match its manifest", () => {
  const config = { manifest: { chainId: "SN_SEPOLIA" } } as ReturnType<
    typeof loadConfig
  >;
  ...
  assert.throws(
    () => requestChainId(config, mainnetRequest),
    /network_mode_mismatch/,
  );
});
test("API errors preserve safe provider/database reasons without exposing hashes", () => {
  assert.equal(
    safeError({ message: "identity_already_linked" }),
    "identity_already_linked",
  );
  assert.equal(
    safeError(new Error(`failed ${"a".repeat(64)}`)),
    "failed [REDACTED]",
  );
});
test("configuration requires a valid pending-delivery private key", () => {
  ...
  assert.throws(
    () => loadConfig({ ...base, PENDING_DELIVERY_PRIVATE_KEY: "short" }),
    /PENDING_DELIVERY_PRIVATE_KEY/,
  );
  assert.throws(
    () =>
      loadConfig({
        ...base,
        STARKNET_NETWORK: "sepolia",
        DEPLOYMENT_MANIFEST_PATH: new URL(
          "../../../deployments/mainnet.json",
          import.meta.url,
        ).pathname,
      }),
    /network_manifest_mismatch/,
  );
  ...
  assert.doesNotThrow(() =>
    loadConfig({ ...base, SOLANA_MAINNET_RPC_URL: undefined }),
  );
  assert.doesNotThrow(() =>
    loadConfig({ ...base, SOLANA_MAINNET_RPC_URL: "" }),
  );
  assert.throws(
    () =>
      loadConfig({
        ...base,
        CCTP_ADMITTED_ROUTES: "solana",
        SOLANA_MAINNET_RPC_URL: undefined,
      }),
    /solana_mainnet_rpc_required/,
  );
  assert.throws(
    () =>
      loadConfig({
  ...
  assert.throws(
    () =>
      loadConfig({
        ...base,
        CCTP_ADMITTED_ROUTES: "solana",
        SOLANA_MAINNET_RPC_URL: "https://api.devnet.solana.com",
      }),
    /solana_mainnet_rpc_required/,
  );
  assert.throws(
    () =>
      loadConfig({
        ...
        BASE_MAINNET_RPC_URL: undefined,
      }),
    /base_mainnet_rpc_required/,
  );
  assert.throws(
    () => loadConfig({ ...base, CCTP_ADMITTED_ROUTES: "ethereum" }),
    /unsupported_admitted_route/,
  );
});
test("wallet challenges bind only an allowlisted dapp origin", () => {
  const config = {
    corsOrigins: ["http://localhost:3000", "https://app.wotta.example"],
  } as ReturnType<typeof loadConfig>;
  assert.equal(
    requireWalletOrigin(config, "http://localhost:3000"),
    "http://localhost:3000",
  );
  assert.equal(
    requireWalletOrigin(config, "http://localhost:3000/"),
  ...
  assert.throws(
    () => requireWalletOrigin(config, "http://localhost:8787"),
    /origin_invalid/,
  );
...
test("wallet binding challenge stays within Ready's proven SNIP-12 type subset", () => {
  ...
  assert.deepEqual(
    new Set(typedData.types.WottaWalletBinding!.map((field) => field.type)),
    new Set(["felt", "shortstring"]),
  );
...
test("inbox key rotation is rejected unless explicitly confirmed", async () => {
  const existing = {
    id: "binding-1",
...
test("mainnet: ethereum, arbitrum, stellar always return coming_soon", () => {
  ...
  for (const id of ["ethereum", "arbitrum", "stellar"] as const) {
    ...
    assert.equal(
      route?.reason,
      "coming_soon",
      `${id} should have coming_soon reason`,
    );
  }
});
...
test("mainnet: base and solana return awaiting_route_evidence after deployment but before admission", () => {
  ...
  for (const id of ["base", "solana"] as const) {
    ...
    assert.equal(
      route?.reason,
      "awaiting_route_evidence",
      `${id} should have awaiting_route_evidence`,
    );
  }
});
...
test("testnet: base and solana never return coming_soon (that reason is mainnet-only)", () => {
  ...
  for (const id of [
    ...
    assert.notEqual(
      route?.reason,
      "coming_soon",
      `${id} must not return coming_soon on testnet`,
    );
  }
});
...
function makeWorkerConfig(overrides: {
  ...
  const relayerAddress =
    overrides.relayerAddress === WORKER_CONFIG_SENTINEL
      ? undefined
      : (overrides.relayerAddress ?? "0x1234");
  const relayerKey =
    overrides.relayerKey === WORKER_CONFIG_SENTINEL
      ? undefined
      : (overrides.relayerKey ?? "0xabcd");
  const fallbackRpc =
    overrides.fallbackRpc === WORKER_CONFIG_SENTINEL
      ? undefined
  ...
  return {
    manifest: {
      ...
      router: {
        verification: {
          status: (overrides.routerVerified ?? true) ? "verified" : "pending",
        },
      },
      pools: [
        {
          denomination: "1000000",
          verification: {
            status: (overrides.poolsVerified ?? true) ? "verified" : "pending",
          },
        },
      ],
    },
    admittedCctpRoutes: new Set(overrides.admittedRoutes ?? ["base"]),
    runtimeVerifiedCctpRoutes: new Set(
      overrides.runtimeVerified === false
        ? []
        : (overrides.admittedRoutes ?? ["base"]),
    ),
    env: {
...
test("assertMainnetWorkerReadiness: testnet is a no-op", () => {
  const config = makeWorkerConfig({
    ...
    poolsVerified: false,
    admittedRoutes: [],
    relayerAddress: WORKER_CONFIG_SENTINEL,
    relayerKey: WORKER_CONFIG_SENTINEL,
    fallbackRpc: WORKER_CONFIG_SENTINEL,
  });
  // Must not throw
...
test("assertMainnetWorkerReadiness: rejects when manifest not verified", () => {
  ...
  assert.throws(
    ...
    /mainnet_worker_manifest_not_verified/,
  );
});
...
test("assertMainnetWorkerReadiness: rejects when router not verified", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /mainnet_worker_router_not_verified/,
  );
});
...
test("assertMainnetWorkerReadiness: rejects when a pool is not verified", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /mainnet_worker_pool_not_verified/,
  );
});
...
test("assertMainnetWorkerReadiness: rejects when no admitted CCTP routes", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /mainnet_worker_no_admitted_routes/,
  );
});
...
test("assertMainnetWorkerReadiness: rejects when relayer credentials missing", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /mainnet_worker_relayer_credentials_missing/,
  );
});
...
test("assertMainnetWorkerReadiness: rejects when fallback RPC absent", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /mainnet_worker_fallback_rpc_required/,
  );
});
...
test("assertMainnetWorkerReadiness: requires distinct mainnet relayer and RPC operators", () => {
  assert.throws(
    () =>
      assertMainnetWorkerReadiness(
        makeWorkerConfig({ deployerAddress: "0x1234" }),
      ),
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(config),
    /fallback_operator_required/,
  );
});

test("assertMainnetWorkerReadiness: requires both workers and verified source RPCs", () => {
  assert.throws(
    () => assertMainnetWorkerReadiness(makeWorkerConfig({ workerPair: false })),
    /worker_pair_required/,
  );
  assert.throws(
    () =>
      assertMainnetWorkerReadiness(
        makeWorkerConfig({ runtimeVerified: false }),
      ),
    /source_rpc_unverified/,
  );
});
...
test("assertMainnetWorkerReadiness: private escrow requires an indexer and independent fallback RPC", () => {
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(noIndexer),
    /mainnet_private_indexer_required/,
  );

  const noFallback = makeWorkerConfig({
    admittedRoutes: [],
    workerPair: false,
    fallbackRpc: WORKER_CONFIG_SENTINEL,
  });
  ...
  assert.throws(
    () => assertMainnetWorkerReadiness(noFallback),
    /mainnet_worker_fallback_rpc_required/,
  );
});
...
test("mainnet fallback RPC must be live and bound to SN_MAIN before workers start", async () => {
  ...
  await assert.doesNotReject(() =>
    assertMainnetFallbackRpcNetwork(config, async () => "0x534e5f4d41494e"),
  );
  await assert.rejects(
    () =>
      assertMainnetFallbackRpcNetwork(config, async () => {
...
test("mainnet source RPC probes bind Base and Solana to their real networks", async () => {
  ...
  assert.deepEqual([...config.runtimeVerifiedCctpRoutes].sort(), [
    "base",
    "solana",
  ]);

  const wrongBase = (async () =>
    new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result: "0x14a34" }), {
      status: 200,
    })) as typeof fetch;
  await assert.rejects(
    () => assertMainnetSourceRpcNetworks(config, wrongBase),
    /base_rpc_network_mismatch/,
  );
...
test("pilot config parses per-tx cap and paused routes", () => {
  assert.throws(
    () => loadConfig({ ...mainnetEnv, PILOT_MAX_USDC_PER_TX: "abc" }),
    /PILOT_MAX_USDC_PER_TX/,
  );
  const config = loadConfig({
    ...mainnetEnv,
...
test("CCTP quote path enforces transfer cap and per-route pause", () => {
  ...
  assert.throws(
    () =>
      assertCctpPilotControls(config, {
        routeId: "solana",
...
test("admitting Base independently does not enable Solana", () => {
  ...
  assert.equal(
    routes.find((route) => route.id === "solana")?.reason,
    "awaiting_route_evidence",
  );
});
...
test("pausing Base does not change Solana capability", () => {
  ...
  assert.equal(
    routes.find((route) => route.id === "base")?.reason,
    "route_paused",
  );
