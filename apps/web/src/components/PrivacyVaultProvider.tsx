...
type PrivacyVaultContextValue = {
  ...
  unlock: () => Promise<{
    vault: PrivacyVault;
    account: Awaited<ReturnType<typeof connectReady>>["account"];
  }>;
  clearVault: () => void;
};

const PrivacyVaultContext = createContext<PrivacyVaultContextValue | null>(
  null,
);
...
export function PrivacyVaultProvider({ children }: { children: ReactNode }) {
  ...
  const [inboxLinkStatus, setInboxLinkStatus] =
    useState<InboxLinkStatus>("checking");
    ...
    void (async () => {
      try {
        ...
        if (
          ...
          error.message.includes("network_mode_mismatch")
        ) {
  ...
  const unlock = useCallback(async () => {
    ...
    try {
      ...
      const next = await unlockPrivacyVault(
        connected.account,
        privacyVaultUnlockConfig(mode),
      );
      setVault(next);
  ...
  const clearVault = useCallback(() => {
    ...
    setInboxLinkStatus("checking");
    clearReadyConnections();
    // Sign-out locks the vault but must not destroy its encrypted inbox key.
    // The server binding survives sign-out and depends on this local secret.
    lockAllPrivacyVaults();
  }, []);
  ...
  return (
    <PrivacyVaultContext.Provider value={value}>
      {children}
