...
type LinkedMe = Awaited<
  ReturnType<ReturnType<typeof createBrowserProductSession>["me"]>
>;
...
export function WalletConnectModal({
  ...
  async function chooseReady() {
    ...
    setPhase("connecting");
    try {
      // A reconnect must discard the account that caused the mismatch. Reusing
      // it makes the recovery dialog loop on the same stale Ready address.
      const connected = await connectReady(mode, { forceReconnect: reconnect });
      operation.assertActive();
      setLinkedAddress(connected.address);
      if (
        reconnect &&
        linkedWalletAddress &&
        BigInt(connected.address) !== BigInt(linkedWalletAddress)
      ) {
        throw new Error(
  ...
  async function bindAndRegister() {
    ...
    try {
      ...
      if (mode === "mainnet") {
        ...
        setStep("done");
        toast.success(
          reconnect ? TOAST.readyMainnetReconnected : TOAST.readyMainnetLinked,
        );
      ...
      setStep("done");
      toast.success(
        reconnect ? TOAST.readyReconnected : TOAST.readyLinkedWithIdentity,
      );
      await onLinked(linkedMe);
    } catch (error) {
      const raw = error instanceof Error ? error.message : String(error ?? "");
      if (raw.includes("wallet_inbox_key_mismatch")) {
        setKeyMismatch(true);
        setPhase("idle");
        return;
      }
      toast.error(
        userFacingError(
          error,
          mode === "mainnet"
            ? TOAST.connectReadyFailed
            : TOAST.privateRegistrationFailed,
        ),
      );
      setPhase("idle");
    } finally {
