...
export function requestWalletReconnect(detail: WalletReconnectRequest = {}) {
  ...
  window.dispatchEvent(
    new CustomEvent(WALLET_RECONNECT_REQUEST_EVENT, { detail }),
  );
}
...
export function isPrivacyReconnectError(error: unknown): boolean {
  ...
  return (
    ...
    lower.includes("connect the ready account linked") ||
    lower.includes("linked to a different ready account")
  );
}

export function networkReconnectTitle(mode: NetworkMode): string {
  return mode === "mainnet"
    ? "Reconnect Ready on Mainnet"
    : "Reconnect Ready on Sepolia";
}
