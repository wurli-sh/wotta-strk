...
function scaledPercent(input: Scaled): number {
  ...
  if (!Number.isFinite(value) || !Number.isSafeInteger(value)) {
    ...
    return (
      Number((integer * 1_000_000n) / 10n ** BigInt(input.decimals)) / 10_000
    );
  }
  return (value / decimals) * 100;
}

export function parseVesuMarket(
  payload: unknown,
  now = new Date(),
): VesuMarketStats {
  ...
  const market = data?.find(
    (item) =>
      sameFelt(item.pool?.id, config.poolAddress) &&
      sameFelt(item.address, config.underlyingAddress),
  );
  if (
    !market ||
    market.pool?.isDeprecated ||
    !sameFelt(market.vToken?.address, config.vTokenAddress)
  )
    throw new Error("vesu_market_unavailable");
  const observedAt = market.config?.lastUpdated
    ? new Date(market.config.lastUpdated)
    : now;
  return {
...
export async function fetchVesuMarket(options?: {
  signal?: AbortSignal;
  now?: Date;
  force?: boolean;
}): Promise<VesuMarketStats> {
  const now = options?.now ?? new Date();
  if (
    !options?.force &&
    cache &&
    now.getTime() - cache.updatedAt.getTime() < CACHE_MS
  )
    return cache;
