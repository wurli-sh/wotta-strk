...
export function formatPrivacySubmitError(error: unknown): string {
  if (!(error instanceof Error)) return "privacy submission failed";
  const message = error.message.trim();
  if (!message) return "privacy submission failed";

  const rpcTail = message.match(/-3260\d:\s*([^"\n]+)/i)?.[1]?.trim();
  if (rpcTail) return truncateSubmitError(rpcTail);

  if (/EMPTY_PROOF_FACTS/i.test(message)) {
    return "Privacy pool received an empty proof — retry with a proof-aware Sepolia RPC";
  }
  if (/invalid signature/i.test(message)) {
    return "Ready privacy authorization signature did not verify — reconnect from Account";
  }
  ...
  return "Privacy proof submission was rejected by Starknet Sepolia";
}
