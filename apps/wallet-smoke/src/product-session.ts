...
export function createProductSession(config: SmokeConfig) {
  if (!config.productApi) return undefined;
...
type FundingStage = "resolving" | "quoting" | "delivering" | "connecting_source" | "approving" | "burning" | "confirming" | "settling";
type RouteManifest = {
...
class WottaProductSession {
  ...
  async connectProvider(provider: AuthProvider) {
    ...
    if (session) {
      ...
      const { error } = await this.supabase.auth.linkIdentity({ provider, options: { redirectTo: this.redirectTo() } });
      if (error) {
        window.sessionStorage.removeItem(pendingProviderKey);
    ...
    window.sessionStorage.setItem(pendingProviderKey, provider);
    const { error } = await this.supabase.auth.signInWithOAuth({ provider, options: { redirectTo: this.redirectTo() } });
    if (error) {
      window.sessionStorage.removeItem(pendingProviderKey);
      throw error;
    }
  ...
  async signIn(provider: "google" | "x") {
    ...
    const { error } = await this.supabase.auth.signInWithOAuth({ provider, options: { redirectTo } });
    if (error) throw error;
  }
  ...
  async fundFromCircleSource(input: {
  ...
  }): Promise<{ intentId: string; sourceTxHash: string; claimSecret: string; escrow: string; expiresAt: string; escrowed: true }> {
    ...
    return { intentId, sourceTxHash: result.txHash, claimSecret, escrow: escrow.address, expiresAt, escrowed: true };
  }
  ...
  async loadLatestClaim(vault: PrivacyVault): Promise<{
    noteId: string;
    claimSecret: string;
