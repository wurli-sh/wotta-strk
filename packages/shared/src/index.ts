...
export const walletLinkSchema = z.object({ challenge: z.string().min(1).max(16_384), signature: z.array(z.string().min(1)).min(1), inboxPublicKey: z.string().regex(/^[A-Za-z0-9_-]{43}$/), rotateInboxKey: z.boolean().optional() });
